...
Perl_av_extend(pTHX_ AV *av, I32 key)
{
	...
	Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(EXTEND), G_DISCARD, 1,
			    arg1);
	return;
    }
...
{             
	...
	Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(PUSH), G_DISCARD, 1,
			    val);
	return;
    }
...
Perl_av_pop(pTHX_ AV *av)
{
	...
	Perl_croak_no_modify();
    if ((mg = SvTIED_mg((const SV *)av, PERL_MAGIC_tied))) {
	retval = Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(POP), 0, 0);
	if (retval)
...
Perl_av_unshift(pTHX_ AV *av, I32 num)
{
    ...
    if ((mg = SvTIED_mg((const SV *)av, PERL_MAGIC_tied))) {
	Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(UNSHIFT),
...
Perl_av_shift(pTHX_ AV *av)
{
	...
	Perl_croak_no_modify();
    if ((mg = SvTIED_mg((const SV *)av, PERL_MAGIC_tied))) {
	retval = Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(SHIFT), 0, 0);
	if (retval)
	    retval = newSVsv(retval);
	return retval;
    }
...
Perl_av_fill(pTHX_ AV *av, I32 fill)
{
	...
	Perl_magic_methcall_sv(aTHX_ MUTABLE_SV(av), mg, SV_CONST(STORESIZE), G_DISCARD,
			    1, arg1);
	return;
    }
    if (fill <= AvMAX(av)) {
